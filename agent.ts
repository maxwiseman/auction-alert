...
import {
	getConversationCriteria,
	readDefaultCriteria,
	updateConversationCriteria,
...
const personalityPrompt = readFileSync(
	...
	return new ToolLoopAgent({
		...
		tools: {
			listFilteredAuctions: tool({
				...
				inputSchema: z.object({
					country: z
						...
						.optional()
						.describe(
							"ISO 3166-1 alpha-2 country code, e.g. US or CA. Defaults to app config.",
						),
					maxBidUsd: z
						.number()
						.int()
						.positive()
						.optional()
						.describe("Maximum current bid in USD. Defaults to app config."),
					maxHoursRemaining: z
						.number()
						.positive()
						.optional()
						.describe("Maximum hours remaining. Defaults to app config."),
					minYear: z
						.number()
						.int()
						.min(1885)
						.max(2100)
						.optional()
						.describe("Minimum model year to include, e.g. 1960."),
					maxYear: z
						.number()
						.int()
						.min(1885)
						.max(2100)
						.optional()
						.describe("Maximum model year to include, e.g. 1989."),
					origins: z
						.array(
							z.enum([
								"American",
						...
						.optional()
						.describe(
							"BaT Origin picker values to include, e.g. British, German, Italian, Japanese, or American.",
						),
					limit: z
						.number()
						.int()
						...
						.optional()
						.describe(
							"Maximum number of auctions to return. Defaults to app config.",
						),
				}),
			...
			readUserCriteria: tool({
				description:
					"Read this conversation's subjective car criteria from Upstash, falling back to the default markdown criteria.",
				inputSchema: z.object({}),
			...
			updateUserCriteria: tool({
				...
				inputSchema: z.object({
					criteria: z
						...
						.min(20)
						.describe(
							"The complete revised criteria text to save for this conversation. Include previous criteria that should remain.",
						),
				}),
				execute: async ({ criteria }) => {
					const settings = await updateConversationCriteria(
			...
			tapback: tool({
				description:
					"Add an iMessage tapback reaction to the current conversation when a quick acknowledgement is better than text.",
				inputSchema: z.object({
					messageId: z
						.string()
						.optional()
						.describe(
							"Message ID to react to. Omit to react to the current inbound message when supported.",
						),
					reaction: z.enum([
						"love",
						"like",
				...
				execute: async ({ messageId, reaction }) => {
					if (!runtime.chat || !runtime.threadId)
						return { ok: false, reason: "No active SendBlue thread." };
					const adapter = runtime.chat.getAdapter?.("sendblue") as
						| SendblueLike
						| undefined;
					if (!adapter?.addReaction)
						return {
							ok: false,
							reason: "SendBlue adapter does not expose addReaction.",
						};
					await adapter.addReaction(
						runtime.threadId,
						messageId ?? runtime.threadId,
						reaction,
					);
					return { ok: true };
				},
			...
			doNotRespond: tool({
				description:
					"Use in group chats when the latest message is not directed at the AI and does not need an Auction Alert response.",
				inputSchema: z.object({
					reason: z
						.string()
						.describe(
							"Brief reason for staying silent. This is logged only and is not sent to the chat.",
						),
				}),
...
export async function readUserCriteria(conversationId?: string) {
	return conversationId
		? getConversationCriteria(conversationId)
		: readDefaultCriteria();
}
...
function extractBatUrlsFromText(text: string) {
	...
	const urls: string[] = [];
	text.replace(
		/https:\/\/bringatrailer\.com\/listing\/[a-z0-9-_/]+/gi,
		(url) => {
